...
function db(prev: { valid: boolean | null; alertedAt: Date | null } | null) {
  ...
  const prisma = { naverBotSession: { findUnique: vi.fn().mockResolvedValue(prev), update } } as unknown as PrismaClient;
  return { prisma, update };
}

describe('reportSessionCheck — 만료 전이 (#9 PR4)', () => {
  it('유효 → 만료: expired (알림 대상)', async () => {
    ...
    expect(update).toHaveBeenCalledWith({ where: { id: 1 }, data: { checkedAt: expect.any(Date), valid: false, checkMessage: '로그인 페이지로 이동' } });
  });
  ...
  it('만료 → 유효: recovered, alertedAt 초기화', async () => {
    ...
    expect(update).toHaveBeenCalledWith({ where: { id: 1 }, data: { checkedAt: expect.any(Date), valid: true, checkMessage: null, alertedAt: null } });
  });
