...
/** 약관 관리 (#252 #297) — 한 종류의 버전·등록 날짜 목록 + 작성/수정(마크다운). 종류는 탭이 정한다 */
export function PoliciesView({ type }: { type: PolicyType }) {
  const trpc = useTRPC();
  const queryClient = useQueryClient();
  const { data: all, isPending } = useQuery(trpc.policy.adminList.queryOptions());
  const data = all?.filter((row) => row.type === type);
  const EMPTY: Draft = { id: null, type, version: '', publishedAt: today(), body: '' };
  ...
  //  현재(최신 등록일) 버전 id — 배지 표시용. adminList 는 publishedAt desc 정렬
    ...
    <div className="flex flex-col gap-4 py-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          가장 최근 등록일의 버전이 현재 {TYPE_LABEL[type]}으로 공개됩니다. 본문은 마크다운(GFM)입니다.
        </p>
        <Button size="sm" onClick={() => setDraft(EMPTY)}><Plus className="size-4" /> 새 버전</Button>
      </div>
      <Table>
        <TableHeader>
          <TableRow>
        ...
        <TableBody>
          {(data ?? []).length === 0 ? (
            <TableRow><TableCell colSpan={4} className="py-10 text-center text-muted-foreground">등록된 버전이 없습니다.</TableCell></TableRow>
          ) : (
            data!.map((policy) => (
              <TableRow key={policy.id}>
                <TableCell>
                  <a href={`/policy/${TYPE_SLUG[type]}/${policy.id}`} target="_blank" rel="noopener noreferrer" className="font-medium hover:underline">
                    {policy.version}
                  </a>
                </TableCell>
                <TableCell className="text-muted-foreground">{new Date(policy.publishedAt).toLocaleDateString('ko-KR')}</TableCell>
                <TableCell>
                  {currentId === policy.id && <Badge>현재</Badge>}
                </TableCell>
                <TableCell>
                  <div className="flex items-center gap-1">
                    <Button variant="ghost" size="icon" aria-label="수정" onClick={() => setDraft({ id: policy.id, type, version: policy.version, publishedAt: toDateInput(policy.publishedAt), body: policy.body })}>
                    ...
                    <Button
                      ...
                      aria-label="삭제"
                      onClick={() => {
                        if (confirm(`${TYPE_LABEL[type]} 버전 ${policy.version} 을(를) 삭제할까요?`)) run(remove.mutateAsync({ id: policy.id }), '삭제했습니다.');
                      }}
                    >
      ...
      <Dialog open={draft !== null} onOpenChange={(open) => !open && setDraft(null)}>
        <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>{TYPE_LABEL[type]} {draft?.id ? '수정' : '새 버전'}</DialogTitle>
            ...
            <div className="flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <Input id="policy-date" type="date" value={draft.publishedAt} onChange={(e) => setDraft({ ...draft, publishedAt: e.target.value })} />
                </div>
              </div>
