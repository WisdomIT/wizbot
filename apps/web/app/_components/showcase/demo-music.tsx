...
export function DemoMusic() {
  ...
  return (
    <div className="grid h-full grid-rows-[auto_minmax(0,1fr)] gap-3 overflow-hidden bg-muted/40 p-3 md:grid-cols-[23.75rem_minmax(0,1fr)] md:grid-rows-1">
      {/* OBS 방송 화면 모사 — 위쪽에 브라우저 소스(자막)가 얹힌다 */}
      <div className="flex min-h-0 flex-col gap-1.5 md:order-2">
        ...
        <div className="relative min-h-0 flex-1 overflow-hidden rounded-lg border bg-[radial-gradient(circle_at_70%_20%,#312e81_0,#0f172a_55%,#020617_100%)]">
          ...
          {/* 브라우저 소스: 상단의 가로로 길고 낮은 자막 영역 — 실제 SongOverlay, 글자는 프레임에 맞게 작게 */}
          <div className="absolute inset-x-4 top-10 h-8 rounded border border-dashed border-white/25 sm:h-9">
            <div className="relative size-full">
              <SongOverlay now={hasSong ? { title: song.title, status: status === 'PLAYING' ? 'PLAYING' : 'PAUSED' } : null} setting={{ mode: 'ALWAYS', durationSeconds: 10 }} fontSize="clamp(13px, 1.4vw, 22px)" />
            </div>
          </div>
        </div>
      </div>
