  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        "group-data-[collapsible=icon]:hidden",
        showOnHover &&
          "peer-data-[active=true]/menu-button:text-sidebar-active-foreground group-focus-within/menu-item:opacity-100 group-hover/menu-item:opacity-100 data-[state=open]:opacity-100 md:opacity-0",
        className
      )}
      {...props}
    />
...
function SidebarMenuSkeleton({
  ...
  //  인스턴스 id 에서 50~90% 폭을 유도한다 — 렌더 중 Math.random 호출 제거 (#200)
  const id = React.useId()
  const width = React.useMemo(() => {
    let hash = 0
    for (const ch of id) hash = (hash * 31 + ch.charCodeAt(0)) % 41
    return `${hash + 50}%`
  }, [id])
